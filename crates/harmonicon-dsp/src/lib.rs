...
//! Pitch detection: five interchangeable algorithms (FFT peak-picking, YIN,
//! pYIN, MPM, template NMF) over one shared FFT, plus the note-template
//! dictionary NMF decomposes against, and the analysis window everything
//! chunks audio by.
//!
//! Deliberately Bevy-free. Improving detection quality means running the
//! algorithms over recordings and comparing hit/miss/phantom counts
//! (`harmonicon-bench`), and that wants a 33-crate dependency tree rather
//! than the engine's 530. The ECS-facing wrappers — `PitchRange` as a
//! `Resource`, `PitchEvent` as a `Message`, `AudioFrame` — live in
//! `harmonicon-audio`, which is also where the tracing spans went: they are
//! Bevy's, and the hot paths they covered are all entered from there.

/// Samples per analysed block. One FFT covers this many samples.
pub const CHUNK_SIZE: usize = 4096;

/// Consecutive chunks overlap by this many samples (50%) — the number of
/// genuinely *new* samples each chunk contributes. Live capture and the
/// offline benchmark both step by this, so their results are comparable.
